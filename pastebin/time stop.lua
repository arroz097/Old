...
local shakescript = script:WaitForChild('shake')
...
function camshake(duration, intensity)
	script.shake.duration.Value = duration
	script.shake.intensity.Value = intensity
	for i,v in pairs(game.Players:GetChildren()) do
	local shako = shakescript:Clone()
	shako.Parent = v.PlayerGui
	shako.Disabled = false
	game.Debris:AddItem(shako, 3)
	end
end
